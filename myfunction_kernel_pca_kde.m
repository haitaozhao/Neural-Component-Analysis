...
XT=(XT-ones(size(XT,1),1)*mean1)./(ones(size(XT,1),1)*std1);%新的故障数据集在标准化时作为减数的均值和作为除数的标准差都取自训练集
...
for i=1:size(Ktest,1)
    t2(i)=Ktest(i,:)*P*inv(diag(LATENT(1:k)))*P'*Ktest(i,:)'; %计算t2统计量   
...
for i=1:size(TKtest,1)
    XTt2(i)=TKtest(i,:)*P*inv(diag(LATENT(1:k)))*P'*TKtest(i,:)'; %计算t2统计量   
